...
//网站根目录
const char* doc_root="/home/nowcoder/mywebserver/resources";
...
void http_conn:: init() {
    ...
    m_linger=false;//默认不保持链接
    m_check_idx = 0; //当前已经解析的字符数
...
//非阻塞的读,循环读取客户端的数据，直到无数据可读或者对方关闭连接
bool http_conn::read(){
    // 更新超时时间
    if(timer) {             
        time_t curr_time = time( NULL );
...
bool http_conn::write() {
    ...
    while(1) {
        ...
        if (temp <= -1) {
            ...
            //虽然在此期间服务器无法立即接收到同一客户的下一个请求，但可以保证连接完整性。
            if (errno == EAGAIN) {
...
http_conn::HTTP_CODE http_conn::parse_request_line(char* text){
    ...
    // GET\0/index.html HTTP/1.1，此时text到\0结束，表示 GET\0
    ...
    *m_version++ = '\0'; // /index.html\0HTTP/1.1，此时m_url到\0结束，表示 /index.html\0
    ...
    if (strncasecmp(m_url,"http://",7) == 0) {
        m_url += 7;     // m_url为192.168.15.128.1:9999/index.html
        m_url = strchr(m_url,'/');
        //在参数 m_url 所指向的字符串中搜索第一次出现字符 '/'的位置。
        // /index.html 
    }
    if ( !m_url || m_url[0]!='/') {
        return BAD_REQUEST;
...
//没有真正解析HTTP请求的消息体，而只是判断它是否被完整读入
...
内存地址m_file_address处， 并告诉调用者获取文件成功
*/
http_conn::HTTP_CODE http_conn::do_request() {
    //"/home/nowcoder/mywebserver/resources"
    ...
    close(fd);
    return FILE_REQUEST;//文件请求，文件获取成功
}
...
bool http_conn::add_response(const char* format,...) {
    ...
    va_start(arg_list, format); //与va_end配合使用
    //vsnprintf用于格式化字符串并将生成的字符存储在缓冲区中。返回写入的字符数
...
//由线程池中的工作线程调用，这是处理HTTP请求的入口函数（在线程池里调用）

...
public:  
    ...
    */
    enum HTTP_CODE { NO_REQUEST, GET_REQUEST, 
                BAD_REQUEST, NO_RESOURCE, FORBIDDEN_REQUEST, 
...
private:
    ...
    sockaddr_in m_address;//通信的soket地址

    char m_read_buf[READ_BUFFER_SIZE];//读缓冲区
    int m_read_idx;//标识读缓冲区以及读入的客户端数据的最后一个字节的下标
    ...
    struct stat m_file_stat;//目标文件的状态。可以判断文件是否存在，是否为目录，是否可读，并获取文件大小等信息
    char* m_file_address; //客户请求的目标文件被mmap到内存中的位置
    struct iovec m_iv[2];//指针成员iov_base指向一个缓冲区
    //这个缓冲区是存放的是readv所接收的数据或是writev将要发送的数据。
    //成员iov_len在各种情况下分别确定了接收的最大长度以及实际写入的长度。
    //用writev来执行写操作，readv和writev函数用于在一次函数调用中读、写多个非连续缓冲区。
    //有时也将这两个函数称为散布读（scatter read）和聚集写（gather write）
    int m_iv_count;//所以定义了m_iv_count表示被写内存块的数量
    

    char m_write_buf[WRITE_BUFFER_SIZE];//写缓冲区
    int m_write_idx;//标识写缓冲区以的待发送的字节数
